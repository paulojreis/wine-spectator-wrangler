var _               = require('lodash'),
    wranglerModule  = {
        ...
        getWineriesPerCountry   : getWineriesPerCountry,
...
function getWineriesPerCountry (rawData) {
    return _(rawData)
                .pluck('ranking')
                .flatten()
                .map(function (value) {
                ...
                .reduce(function (accum, value, index, collection) {
                    if (!_.has(accum, value.country)) {
                        accum[value.country] = [];
                    } 

                    if (_.indexOf(accum[value.country], value.winery_full) === -1) {
                        accum[value.country].push(value.winery_full);                                            
                    }
